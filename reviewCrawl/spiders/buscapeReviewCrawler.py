...
class BuscapeReviewSpider(scrapy.Spider):
    ...
    def parse_product(self, response):
        ...
        ''' 
            ...
            if 'pagina' in reviews_page:
                #versao antiga
                self.per_page = 5
                yield scrapy.Request(
                        response.urljoin(reviews_page),
                        callback=self.parse_reviews_DEPRECATED
                )
...
# ---------------------- FORMA ANTIGA. SERA MANTIDA ENQUANTO EXISTIREM LINKS DESATUALIZADOS --------------------------
    def parse_product_DEPRECATED(self, response):
    ...
    def parse_reviews_DEPRECATED(self, response):
